...
object ultimaDeclaracionVigenteLand extends TSparkSession {
  ...
  def ultimaDeclaracionVigenteLandRun(
    ...
    try {
      ...
      case analysisEx: AnalysisException => logger.error(analysisEx.getMessage())
    }
    try {
    ...
    try {
      ...
      spark
        .sql(
          s"""SELECT B.partitionkey, B.rowkey, A.timestamp, B.concepto, B.rfc, A.numerooperacion, B.fechadeclaracion,
  ...
  def ingestaRun(
    ...
    try {
      ...
      val complementoConsulta = s"""Ejercicio ge ${ConfigurationProvider.ejercicioDeclaracion.toString} and IdEstadoProceso eq ${ConfigurationProvider.idEstadoProceso.toString} and Timestamp ge datetime'${rangoFechas._1.replace(" ", "T")}' and Timestamp le datetime'${rangoFechas._2.replace(" ", "T")}'"""

      val storageAzureConcepto = new AzureTable(secretScope, watConsumoConcepto)
      val tablaLandCompleta = storageAzureConcepto
        .queryDeclaracionAnual(complementoConsulta)
        .asScala
        .toList
        .map(a => {
      ...
      val complementoConsultaDetalle = s"""Timestamp ge datetime'${rangoFechas._1.replace(" ", "T")}' and Timestamp le datetime'${rangoFechas._2.replace(" ", "T")}'"""

      val storageAzureDetalle = new AzureTable(secretScope, watConsumoConceptoDetalle)
      logger.debug(s"consulta:$complementoConsultaDetalle")
      val tablaLandCompletaDetalle = storageAzureDetalle
        .queryDeclaracionAnual(complementoConsultaDetalle)
        .asScala
        .toList
        .map(a => {
          val fechaDeclaracion = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss.SSS")
